...
class BatchProcessor:
    ...
    def register_processor(
        self,
        operation: str,
        processor: Callable[[List[BatchItem]], Any]
    ) -> None:
    ...
    async def _flush_batch(self, operation: str) -> None:
        ...
        self._stats["avg_batch_size"] = (
            (current_avg * (total_batches - 1) + len(items)) / total_batches
        )
    ...
    async def _handle_failed_batch(
        ...
        if retry_items:
            ...
            for operation in set(item.operation for item in retry_items):
                wait_time = self._get_wait_time(operation) * (2 ** retry_items[0].retry_count)
    ...
    def _adapt_parameters(
        ...
        items_per_second = batch_size / processing_time if processing_time > 0 else batch_size
        ...
        logger.debug("Adapted batch parameters",
                    operation=operation,
